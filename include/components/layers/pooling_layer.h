...
public:
    ...
    // Info
    std::string get_type() const override { return "PoolingLayer"; }
    LayerType get_layer_type() const override { 
        return (pool_type_ == MAX) ? LayerType::MAX_POOLING : LayerType::AVERAGE_POOLING;
    }
    ...
    const Eigen::MatrixXd& get_weights_gradient() const override { 
        static Eigen::MatrixXd empty; 
        return empty; 
    }
    
    // Gestione bias (Pooling non ha bias)
    bool get_use_bias() const override { return false; }  // ← AGGIUNTO!
    Eigen::VectorXd get_biases() const override { return Eigen::VectorXd(); }
    void set_biases(const Eigen::VectorXd& biases) override { (void)biases; }
    void set_bias_gradient(const Eigen::VectorXd& gradient) override { (void)gradient; }
    const Eigen::VectorXd& get_bias_gradient() const override { 
